Shader "Hidden/Kino/PostProcessing/Isoline"
{
    ...
    float2 Contour(VaryingsDepthSupport input)
    {
        ...
        float3 wpos = mul(_InverseView, float4(vpos.x, vpos.y, vpos.z, 1)).xyz;

        const float Interval = _ContourParams.x;
        const float Width = _ContourParams.z;
		
        // Potential value and derivatives
        float pot = (dot((_ContourAxis.xyz), wpos) + _ContourAxis.w) / Interval;
		
        float pot2 = (dot((_ContourAxis.zyx), wpos) + _ContourAxis.w) / Interval;
		
		

        // Contour detection
        float fw = fwidth(pot);
        float fww = fw * Width * 1;
        float ct = saturate((abs(  frac(pot) * frac(pot2) ) - 1 + fww) / fww);
    ...
    {
        const float SourceContrib = _ContourParams.w * 2;
